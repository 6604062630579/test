...
const Navbar = () => {
  ...
  const handleCall = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/table/call`,
        { table: tableNumber }
      );
  ...
  return (
    <div className="flex items-center justify-between pt-4 sm:py-5 px-4 sm:px-8 font-medium">
      {location.pathname === "/" ? (
        <div className="flex gap-4 pt-2 ">
          <AlignLeft
            ...
            className="size-6 sm:size-10 cursor-pointer md:hidden text-Text"
          />
          <Link
            to="/"
            className="flex md:flex-col lg:flex-row items-center gap-3"
          >
            <img className="w-16 sm:w-24 md:w-32 " src={assets.logo} alt="" />
            <h1 className="w-36 text-sm xl:text-md sm:text-lg md:text-2xl text-Text md:text-center whitespace-nowrap">
              Table : {tableNumber}
            </h1>
          </Link>
        </div>
        ...
        <div>
          ...
          >
            ...
            <img className="w-16 sm:w-24 md:w-32 " src={assets.logo} alt="" />
            <h1 className="w-36 text-sm xl:text-md sm:text-lg md:text-2xl text-Text text-center whitespace-nowrap">
              Table : {tableNumber}
            </h1>
          </Link>
        </div>
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `flex flex-col items-center gap-1 px-3 py-2 rounded ${
              isActive ? "bg-BG_sec text-white shadow-lg" : ""
            }`
          }
        >
          <p>HOME</p>
      ...
      <div className="flex items-center gap-2 sm:gap-4 md:gap-6 justify-end">
        <div onClick={handleCall} className="cursor-pointer">
          <div className="w-9 h-9 sm:w-12 sm:h-12 bg-BG rounded-full shadow-lg shadow-Text/20 flex items-center justify-center">
            <ConciergeBell
              className="size-6 sm:size-8 text-Text"
              alt="Concierge Bell"
            />
          </div>
        </div>
        <Link to="/orderSummary" className="relative">
          ...
          <p className="absolute right-0 bottom-0 transform translate-x-1/6 translate-y-1/6 w-4 text-center leading-4 bg-BG_Black text-BG aspect-square rounded-full text-[8px] shadow-md">
            {totalFoodCount}
          </p>
        </Link>
        <div onClick={() => setDarkMode(!darkMode)} className="cursor-pointer">
          <div className="w-9 h-9 sm:w-12 sm:h-12 bg-BG rounded-full shadow-lg shadow-Text/20 flex items-center justify-center">
            {darkMode ? (
              <Sun className="size-6 sm:size-8 text-Text" />
            ) : (
              <Moon className="size-6 sm:size-8 text-Text" />
            )}
          </div>
        </div>
      </div>
      ...
      <div
        className={`z-50 absolute top-0 left-0 bottom-0 overflow-hidden bg-BG transition-all  ${
          visible ? "w-full" : "w-0"
        }`}
      >
        <div className="flex flex-col py-4 sm:py-5 px-4 sm:px-9 text-Text">
          ...
          <NavLink
            onClick={() => setVisible(false)}
            className="py-2 pl-6 border border-Text"
            to="/"
          >
            HOME
          </NavLink>
          ...
          <div className="flex flex-col gap-2 mt-6 text-center p-4 bg-BG border border-Text rounded-lg shadow-lg">
            <img
              className="w-16 sm:w-24 md:w-32 mx-auto"
              src={assets.logo}
              alt=""
            />
