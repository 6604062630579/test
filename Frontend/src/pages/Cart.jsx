  ...
  const onSubmitHandler = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/order/add`,
        formData
      );
          ...
          <div>
            {cartData.map((item, index) => (
              ...
              >
                <div
                  data-aos="fade-right"
                  className="flex gap-3 md:gap-6 w-full"
                >
                  <img
                    src={item.image[0]}
                    className="w-28 sm:w-44 rounded-lg"
                    alt=""
                  />
                ...
                <div
                  data-aos="fade-left"
                  className="md:w-[25%] p-2 flex flex-col items-center"
                >
                  <p className="text-lg md:text-2xl ">
                    {item.quantity * item.price}
                  </p>
                  <Trash2
                    className="size-4 md:size-10 cursor-pointer"
                ...
                <div
                  data-aos="fade-left"
                  className="md:w-[25%] flex flex-col gap-2 items-center justify-center"
                >
        ...
        <div className="w-full sm:w-[450px] mt-10">
          ...
          <div className="w-full flex justify-center text-end">
            <Link
              ...
              to="/orderSummary"
              className="w-[90%] text-center select-none rounded-lg bg-Button text-white text-sm my-8 px-8 py-3"
            >
              CHECKOUT
            </Link>
          </div>
