...
const OrderSummary = () => {
  ...
  const handleCheckBill = async () => {
    ...
    toast.info(
      `Bill checked! Grand Total: ${currency} ${totalAmount} | Total Food: ${totalFoodCount}`,
      {
        position: "top-right",
        autoClose: 5000,
  ...
  const fetchOrders = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/order/list/${tableNumber}`
      );
                ...
                {order.products.map((item, idx) => {
                  return (
                    <li
                      data-aos="fade-up"
                      key={idx}
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between py-2 md:pr-20"
                    >
                      <div className="flex items-center justify-center">
                        <img
                          src={item.image[0]}
                          alt={item.name}
                          className="w-32 sm:w-44 object-cover mr-4 rounded-lg"
                        />
                        <div className="flex flex-col gap-2">
                          <span className="text-md md:text-lg md:w-full lg:text-2xl">
                            {item.name} (x{item.quantity})
                          </span>
                          <span
                            className={`w-24 sm:w-28 md:w-36 text-md md:text-lg lg:text-2xl text-center select-none badge-${order.status.toLowerCase()}`}
                          >
                            {order.status}
                          </span>
                          <p className="text-md md:text-lg lg:text-xl truncate max-w-44 sm:max-w-60 md:max-w-80 lg:max-w-96">
                            <b>Additional </b> :{" "}
                            {item.requirement ? item.requirement : "No Req"}
                          </p>
                        </div>
                      </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl w-96">
            <div className="flex items-center justify-between mb-5">
              ...
              <div
                ...
                className="w-7 h-7 sm:w-8 sm:h-8 bg-red-500 hover:bg-red-400 rounded-lg shadow-lg shadow-Text/20 flex items-center justify-center cursor-pointer"
              >
