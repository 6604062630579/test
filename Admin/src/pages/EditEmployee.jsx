...
const EditEmployee = ({ token, role }) => {
  ...
  const fetchEmployee = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/employee/singleStaff`,
        { staffId }
      );

      if (response.data.success) {
        const staff = response.data.staff;
        const addressParts = staff.address
          ? staff.address.split(", ")
          : ["", "", "", ""];
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/employee/updateStaff`,
        formData,
        { headers: { token } }
      );
    ...
    <div className="w-full flex flex-col items-center text-Text p-2 sm:p-8">
      ...
      >
            ...
            <div className="flex gap-4 w-full mb-4">
              <div className="w-full">
                ...
                <input
                  ...
                  value={firstname}
                  type="text"
                  className="w-full rounded-lg p-2 bg-Text/20"
                  required
                />
              </div>
              <div className="w-full">
        ...
        <div className="cols-span-4 lg:cols-span-1 grid rounded-xl gap-3">
          ...
          <button
            type="submit"
            className="rounded-xl bg-Button p-5 text-3xl text-BG hover:bg-Button/90"
          >
            Update Employee
          </button>
        </div>
