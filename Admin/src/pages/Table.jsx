...
const Table = () => {
  ...
  const handleStatusChange = async (index, newStatus, orderId) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/order/update`,
        {
          id: orderId,
          status: newStatus,
        }
  ...
  const handleClearOrder = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/analytics/add`,
        {
          products: products,
          table: tableNumber,
        }
      ...
      if (response.data.success) {
        ...
        setTotalPrice(0);
        await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/api/order/remove`,
          {
                          ...
                          <tbody>
                            {order.products.map((product, productIndex) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-2 w-1/3 border-r">
                                  {product.requirement ||
