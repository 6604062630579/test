...
const Order = () => {
  ...
  const handleOpenTable = async (number) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/table/available`,
        {
          table: number,
          available: true,
        }
  ...
  const confirmDeleteTable = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/table/delete`,
        {
  ...
  const handleAttendTable = async (number) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/table/attend`,
        {
          table: number,
        }
      );
  ...
  return (
    <div className="w-full flex flex-col items-center text-Text p-2 sm:p-8">
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between w-full">
        <h1 className="text-2xl sm:text-4xl font-bold self-start ml-1">
          Order
        </h1>
        <div
          ...
          <div key={table.table} className="flex justify-center">
            ...
            >
              <button
                ...
                className="flex items-center justify-center self-start px-4 py-2 gap-3text-white hover:bg-red-400 bg-red-600 rounded-2xl cursor-pointer ml-3"
              >
                X
              </button>
